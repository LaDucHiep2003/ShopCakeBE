...
class BaseModel
{
    ...
    public function index()
    {
        ...
        $query = $this->conn->prepare("select * from $this->table where deleted = false LIMIT :limit OFFSET :offset ");
...
//    }
    public function delete($id)
    {
        try {
            $this->conn->beginTransaction();
            $query = $this->conn->prepare("UPDATE $this->table SET deleted = true WHERE id = :id");
            $query->execute(['id' => $id]);
            $this->conn->commit();
            return true;
        } catch (Throwable $e) {
